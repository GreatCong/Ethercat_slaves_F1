...
// @ function: 
// @ description：HAL_GPIO_EXTI_Callback
// @ input:
// @ note: no wifi,rw_lib_platform.c copy
...
//	}
	...
	//Ethercat SYNC0
	if(GPIO_Pin == ETHERCAT_SYNC0_Pin){
	  Ethercat_sync0_handle();
	}
	//Ethercat SYNC1
	if(GPIO_Pin == ETHERCAT_SYNC1_Pin){
	  Ethercat_sync0_handle();
	}
}
