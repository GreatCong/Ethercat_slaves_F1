...
//  GPIO_Init(GPIOE, &GPIO_InitStructure);
	...
	//SPI1_Init(); 
	//MX_SPI3_Init();//默认是已经初始化了

} 


//#define SPI_I2S_FLAG_RXNE               ((uint16_t)0x0001) //add lcj
/*******************************************************************************
...
//	return SPI1->DR;
    ...
    while(HAL_SPI_GetState(&hspi1)==HAL_SPI_STATE_BUSY_TX){};//等待发送缓冲区空
	  ...
	  status = HAL_SPI_TransmitReceive(&hspi1,&cmd,&recv,1,5000);
	  assert_param(status == HAL_OK);	
		return recv;
//	SPI1->DR = cmd;        //发送数据
//	while ((SPI1->SR & 1<<0) == 0x00) ;
//	return SPI1->DR;
