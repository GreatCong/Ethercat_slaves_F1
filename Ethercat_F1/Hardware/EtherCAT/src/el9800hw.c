...
-----------------------------------------------------------------------------------------*/
#if AL_EVENT_ENABLED
#define    INIT_ESC_INT           EXTI3_Configuration();					
#define    EcatIsr                EcatIsr//EXTI3_IRQHandler
#define    ACK_ESC_INT         		__HAL_GPIO_EXTI_CLEAR_FLAG(ETHERCAT_IRQ_Pin)//EXTI_ClearITPendingBit(EXTI_Line3);  
#define IS_ESC_INT_ACTIVE					((HAL_GPIO_ReadPin(ETHERCAT_IRQ_GPIO_Port,ETHERCAT_IRQ_Pin))==0) //((GPIO_ReadInputDataBit(GPIOE,ETHERCAT_IRQ_Pin)) == 0) 
...
#define    Sync0Isr                        Sync0Isr//EXTI1_IRQHandler 
#define    DISABLE_SYNC0_INT               NVIC_DisableIRQ(EXTI1_IRQn);	 
#define    ENABLE_SYNC0_INT                NVIC_EnableIRQ(EXTI1_IRQn);	
#define    ACK_SYNC0_INT                   __HAL_GPIO_EXTI_CLEAR_FLAG(ETHERCAT_SYNC0_Pin) //EXTI_ClearITPendingBit(EXTI_Line1);
#define    IS_SYNC0_INT_ACTIVE             ((HAL_GPIO_ReadPin(ETHERCAT_SYNC0_GPIO_Port,ETHERCAT_SYNC0_Pin))==0)//((GPIO_ReadInputDataBit(GPIOC,SYNC0_Pin)) == 0) 

																					
/*ECATCHANGE_START(V5.10) HW3*/

#define    INIT_SYNC1_INT                  EXTI2_Configuration();
#define    Sync1Isr                        Sync1Isr//EXTI2_IRQHandler
#define    DISABLE_SYNC1_INT               NVIC_DisableIRQ(EXTI2_IRQn);
#define    ENABLE_SYNC1_INT                NVIC_EnableIRQ(EXTI2_IRQn); 
#define    ACK_SYNC1_INT                   __HAL_GPIO_EXTI_CLEAR_FLAG(ETHERCAT_SYNC1_Pin)//EXTI_ClearITPendingBit(EXTI_Line2);
#define    IS_SYNC1_INT_ACTIVE             ((HAL_GPIO_ReadPin(ETHERCAT_SYNC1_GPIO_Port,ETHERCAT_SYNC1_Pin))==0)//(GPIO_ReadInputDataBit(GPIOC,SYNC1_Pin)) == 0)  
...
#define ECAT_TIMER_ACK_INT        		 	__HAL_TIM_CLEAR_FLAG(&htim2 , TIM_SR_UIF)//TIM_ClearITPendingBit(TIM2 , TIM_FLAG_Update);	
#define    TimerIsr                    	TimerIsr//TIM2_IRQHandler 					
...
#define STOP_ECAT_TIMER              	HAL_TIM_Base_Stop(&htim2)//TIM_Cmd(TIM2, DISABLE);	

#define START_ECAT_TIMER           		HAL_TIM_Base_Start(&htim2) //TIM_Cmd(TIM2, ENABLE);				
